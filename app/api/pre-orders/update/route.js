...
// Passes a pre-order's status change to the handover app — the record lives
// there, not here. The signed-in person's ID token rides along so it's
// attributable.
...
export async function POST(req) {
  ...
  try {
    const res = await fetch(`${base.replace(/\/$/, "")}/api/pre-orders/update`, {
      method: "POST",
      headers: {
