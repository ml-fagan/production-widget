...
export function onSlot(allocations, machineId, slot) {
  return allocations.find(
    (a) => a.machineId === machineId && Number(a.slot) === slot && isOpen(a)
  );
}

/** Everything that ran on a slot today and has since been closed. */
export function closedOnSlot(allocations, machineId, slot) {
  return allocations
    .filter((a) => a.machineId === machineId && Number(a.slot) === slot && !isOpen(a))
    .sort((a, b) => String(a.startAt).localeCompare(String(b.startAt)));
}

/**
 * The job Decorflow already has booked on a machine that day.
 *
 * The schedule knows what's on the edger on Tuesday. Asking Adam to type it
 * again is asking for the two to disagree, so a new slot starts with this and
 * he changes it only when the floor does something else.
 */
export function bookedJob(bookings, machineId, date) {
  const hit = bookings
    .filter((b) => b.machineId === machineId && b.date === date)
    .sort((a, b) => Number(b.hours || 0) - Number(a.hours || 0))[0];
  return hit ? String(hit.jobId || "") : "";
}
