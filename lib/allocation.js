...
/**
 * Who is on a step now, longest on it first.
 *
 * A step is one cell with a list in it rather than a row of numbered lines:
 * people are added to it and ended off it, and how many it holds is however
 * many are on it.
 */
export function onStep(allocations, stepId) {
  return allocations
    .filter((a) => a.stepId === stepId && isOpen(a))
    .sort((a, b) => String(a.startAt).localeCompare(String(b.startAt)));
}

/** Who was on it earlier today and has since been ended off. */
export function endedOnStep(allocations, stepId) {
  return allocations
    .filter((a) => a.stepId === stepId && !isOpen(a))
    .sort((a, b) => String(a.startAt).localeCompare(String(b.startAt)));
}
