...
import { crmLooksValid } from "./crmUtils.js";
...
function parseBlock(rows, startRow, fc) {
  ...
  for (let r = startRow + 1; r < rows.length; r++) {
    ...
    out.push({
      ...
      dispatch: actual || committed, // actual scheduled-out date takes precedence when set
      committed,
      actual,
      lead,
      priority,
...
export function parseSchedule(buffer) {
  ...
  for (const j of jobs) {
    ...
    seen.add(key);
    // Overdue = the effective dispatch date has passed. The effective date is
    // the actual scheduled-out date when set (a reschedule the client's been
    // told about), otherwise the committed date. A future actual date means the
    // job's been pushed forward and is on track; a past one means it's slipped.
    let overdue = false;
    const effective = j.actual || j.committed;
    if (effective) {
